...
INCLUDEPATH += $$OUT_PWD
HEADERS  += src/webhook.h \
    src/entityhandler.h \
    src/httpmethod.h \
    src/lighthandler.h \
    src/switchhandler.h \
    src/webhookcommand.h \
    src/webhookentity.h \
    src/webhookrequest.h
SOURCES  += src/webhook.cpp \
...
!isEmpty(QMAKE_LUPDATE):exists("$$QMAKE_LUPDATE") {
    ...
    command = $$QMAKE_LUPDATE webhook.pro
    system($$command) | error("Failed to run: $$command")
    command = $$QMAKE_LRELEASE webhook.pro
    system($$command) | error("Failed to run: $$command")
} else {
